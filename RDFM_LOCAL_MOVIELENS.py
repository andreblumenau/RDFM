...
def sgd_subset(train_X, train_Y, iterations, alpha, regularization,weight_matrix):
    ...
    matrix_set_diag_to_zero = sparse.csr_matrix(matrix_set_diag_to_zero)

    for i in range(N):
        tensor_of_x_features[i]=train_X[i].todense()
        tensor_of_x_squared[i]=(train_X[i].dot(train_X[i])).todense()
...
validationX = sparse.csr_matrix(validationX)
